...
uint32_t Get_Distance_1(void) // 传入时间单位10us
{
	uint32_t Distance = 0;
	/*将Trig引脚拉高十微秒*/
...
uint32_t Get_Distance_2(void) {
	...
	while (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_1) == 1)
  {
	...
	return Distance > 2.5 ? Distance : 1;
}
